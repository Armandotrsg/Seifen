...
<nav class={`${state.scrolled ? "bg-slate-100" : "bg-slate-200"} dark:bg-gray-900 fixed w-full z-20 top-0 left-0 border-b border-gray-300 dark:border-gray-600`} window:onScroll$={handleScroll}>
  ...
  <div class={`items-center justify-between ${!state.open ? "hidden" : ""} w-full md:flex md:w-auto md:order-1`} id="navbar-sticky">
    <ul class={`flex flex-col p-4 md:p-0 mt-4 font-medium border border-slate-100 rounded-lg bg-slate-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 ${state.scrolled ? "md:bg-slate-100" : "md:bg-slate-200"} dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700`}>
      <li>
