...
<nav class={`${state.scrolled ? "bg-slate-100 border-b border-gray-300 dark:border-gray-600" : "bg-slate-200"} dark:bg-gray-900 fixed w-full z-20 top-0 left-0  duration-300 ease-in-out`} window:onScroll$={handleScroll}>
  ...
  <div class="flex md:order-2">
      ...
      <button data-collapse-toggle="navbar-sticky" type="button" class="inline-flex items-center p-2 text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-sticky" aria-expanded="false" onClick$={() => state.open = !state.open}>
        <span class="sr-only">Open main menu</span>
        <svg class="w-6 h-6" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 15a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"></path></svg>
      </button>
  </div>
  <div class={`items-center justify-between ${!state.open ? "hidden" : ""} w-full md:flex md:w-auto md:order-1`} id="navbar-sticky">
    <ul class={`flex flex-col p-4 md:p-0 mt-4 font-medium border border-slate-100 rounded-lg bg-slate-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 duration-300 ease-in-out ${state.scrolled ? "md:bg-slate-100" : "md:bg-slate-200"} dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700`}>
      <li>
